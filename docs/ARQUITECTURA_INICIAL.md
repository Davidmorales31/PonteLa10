...
8. Valida la salida y persiste evidencia.
9. Elimina archivos temporales.
10. Finaliza de forma idempotente o registra un error recuperable.

## Estados propuestos

`pending`, `queued`, `extracting`, `transcribing`, `translating`,
`drafting`, `draft_created`, `requires_review`, `failed` y `cancelled`.

## Idempotencia y recuperación

- Identificador único por intento.
- Lease renovable y heartbeat.
- Un artículo máximo por ingesta.
- Un intento vencido no puede finalizar otro intento.
- Creación de artículo y finalización mediante operación transaccional.
