...
export default defineEventHandler(async (evento) => {
  ...
  const solicitud = validarEntradaEditorial(esquemaGenerarBorradorIa, await readBody(evento))
  if (!solicitud.regenerar) throw createError({ statusCode: 409, statusMessage: 'La redacción se genera automáticamente. Usa “Reintentar borrador” solo después de una falla.' })
  ...
  const hash = hashPromptRedaccion(entrada)
  const reserva = await reservarBorradorDesdeIngesta(cliente, ingestaId, requestId, hash)
  ...
  try {
    ...
    return await crearBorradorDesdeIngesta(cliente, ingestaId, requestId, redaccion.propuesta, redaccion, hash)
  } catch (error) {
