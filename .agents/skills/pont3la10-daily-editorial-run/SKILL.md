...
2. Target five to seven strong candidates per active category, not a filler
   quota. Return fewer when sourcing, originality, or reader value is weak.
   Opinión and Especiales stay explicitly flagged for human angle/format review.
...
4. For each candidate, use the checkpoint CLI from the repository root. Save
   ...
   - `portada`: image bytes stored by content hash and editorial metadata.
   - `media`: stable returned media ID and safe upload receipt.
   ...
   generated image plus metadata, and `payload-portada` with an output filename
...
5. Never repeat completed research or image-generation calls on resume. Reuse
   stored dossiers, draft, cover and media ID. For an uncertain API outcome,
   replay the exact saved proposal payload and idempotency key; inspect the API
...
If required API configuration, research access, image generation, or safe
checkpoint artifacts are unavailable, stop the affected candidate at the last
completed stage and report what is needed. Do not fabricate sources, drafts,
images, receipts, or successful API writes.
